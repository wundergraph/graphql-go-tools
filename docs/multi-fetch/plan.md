...
- [ ] **Step 1: Write failing tests** targeting `collectGroups`/`clearMergeableOperations` directly (package-internal), plus two pipeline-level tests. Build flat Sequence trees of `resolve.Single(&resolve.SingleFetch{...})` nodes in the style of `create_parallel_nodes_test.go`. IMPORTANT: every grouping tree also contains a non-candidate root node `FetchID: 0` (empty `DependsOnFetchIDs`, no MergeableOperation) so that candidate dependencies `{0}` are actually provided in the wave simulation. Candidate template: `FetchConfiguration{RequiresEntityBatchFetch: true, MergeableOperation: &resolve.MergeableOperation{Variables: []resolve.NamedVariableFragment{{Name: "representations", Value: []byte("[$$0$$]")}}}, Variables: resolve.NewVariables(resolve.NewResolvableObjectVariable(&resolve.Object{}))}` with `Info: &resolve.FetchInfo{DataSourceID: "ds1"}`. Cases:
  ...
  10. `NewProcessor(EnableMultiFetch(), DisableResolveInputTemplates())` over two valid candidates ⇒ no `MultiEntityFetch` emitted, fetches keep readable `Input` strings, artifacts still cleared. (Pipeline-level cases 9-10: give the two fetches DISTINCT `Input` strings — `deduplicateSingleFetches` runs first and collapses byte-identical configs.)
...
- **Append shape** (input starts with `{"body":{"variables":`): `variablesStart` = position of that `{`; forward brace-balanced quote-aware scan for `variablesEnd`; require `,"query":"` next; `queryStart` after it; `queryEnd` = the UNIQUE q such that `input[q:q+2] == "\"}"` AND `q+2 < len(input)` AND `input[q+2] == ','` AND `input[q-1] == '}'` (a printed operation always ends with a closing selection-set brace; a trailing `"POST"}` at EOF never matches because nothing follows it). Zero matches OR more than one match ⇒ `ok=false` — ambiguity must fail safe: a header value ending in a raw `}` can produce a later false anchor, and a declined merge is always correct while a mis-split corrupts the request.
...
1. Ordered short-circuits:
   ...
   - `if res.err != nil || res.authorizationRejected || res.rateLimitRejected || len(res.out) == 0`: copy `err`, `statusCode`, `ds`, `out`, `rateLimitRejected(+Reason)`, `authorizationRejected(+Reasons)` onto every entry res EXCEPT entries already excluded at prepare (`entryRes.fetchSkipped` set — a fetch that was never going to be sent gets no transport/rate-limit error, matching unmerged behavior); `multi` stays nil; jump straight to the step-4 merge loop (each non-excluded entry's `mergeResult` guards render failed-to-fetch / rate-limit errors at that entry's path); skip steps 2-3. (loadPhase already recorded `erroredFetchIDs` for `res.err` only — invalid responses must NOT cascade, spec 4.7 step 1.)
2. Parse once: `response, err := astjson.ParseBytesWithArena(l.jsonArena, res.out)`; on parse failure, copy `out`/`statusCode`/`ds` to entries (same prepare-time-exclusion carve-out; `multi` stays nil) and run the step-4 merge loop (per-entry status-fallback/invalid-JSON errors, exactly today's guards). On success: collect response extensions once (replicate loader.go:643-649 against `response`).
...
- [ ] **Step 1: Write failing tests** (stub DataSource returning canned bodies; drive through `resolveSingle` on a hand-built multi FetchItem so prepare+load+merge all run):
  ...
  - `TestMergeMultiEntityResult_TransportError`: stub returns error ⇒ one failed-to-fetch error PER non-excluded entry path; `erroredFetchIDs` contains the multi's FetchID. Variant: entry2 excluded at prepare (empty parent data) ⇒ the error appears ONLY at entry1's path (excluded entries were never sent — no error, spec edge rows).
