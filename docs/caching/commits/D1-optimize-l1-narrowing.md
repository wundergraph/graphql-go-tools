...
- For each fetch the stamper already marked L1-eligible, the pass KEEPS `cfg.L1 = true` only when a valid provider/consumer pair justifies it (`canRead || canWrite`), and otherwise turns it off — it never re-enables L1:
  ...
  - `canWrite` = a later fetch of the same type needs a SUBSET AND this fetch contributes at least one of those fields (`hasValidConsumer`);
