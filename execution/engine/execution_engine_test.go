...
func TestExecutionEngine_Execute(t *testing.T) {
  ...
  `)
	...
	t.Run("optional requires dependency fails", func(t *testing.T) {
		definition := `
			type Query {
				accounts: [User!]!
			}
			...
			mustGraphqlDataSourceConfiguration(t,
				...
				mustFactory(t,
					testNetHttpClient(t, roundTripperTestCase{
						...
						expectedPath:     "/",
						expectedBody:     `{"query":"query($representations: [_Any!]!){_entities(representations: $representations){... on User {__typename title}}}","variables":{"representations":[{"__typename":"User","id":"1"},{"__typename":"User","id":"3"}]}}`,
						sendResponseBody: `{"data":{"_entities":[{"__typename":"User","title":null},{"__typename":"User","title":"User3"}]},"errors":[{"message":"Some unrelated warning","locations":[{"line":1,"column":20}],"path":["_entities",1,"title"]},{"message":"Cannot provide value","locations":[{"line":1,"column":30}],"path":["_entities",0,"title"]}]}`,
						sendStatusCode:   200,
					}),
				),
				&plan.DataSourceMetadata{
	...
	t.Run("optional requires dependency fails on nested fields", func(t *testing.T) {
		definition := `
			type Query {
				accounts: [User!]!
			}
