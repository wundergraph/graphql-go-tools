...
func NewProxyEngineConfigFactory(schema *graphql.Schema, proxyUpstreamConfig ProxyUpstreamConfig, opts ...ProxyEngineConfigFactoryOption) *ProxyEngineConfigFactory {
...
func (p *ProxyEngineConfigFactory) EngineConfiguration() (Configuration, error) {
	...
	dataSource, err := newGraphQLDataSourceGenerator(&rawDoc).Generate(
		dataSourceConfig,
		p.httpClient,
