...
const weaviateConfigSDL = `
extend schema @index(name: "products", backend: "weaviate", config: "{}")
...
func TestWeaviate(t *testing.T) {
	...
	makeSetup := func(name, configSDL string) BackendSetup {
		return BackendSetup{
			...
			CreateIndex: func(t *testing.T, name string, schema searchindex.IndexConfig, _ []byte) searchindex.Index {
				...
				configJSON := []byte(fmt.Sprintf(`{"host":%q,"scheme":"http"}`, weaviateHost))
				idx, err := factory.CreateIndex(context.Background(), name, schema, configJSON)
				if err != nil {
	...
	t.Run("standard", func(t *testing.T) {
		t.Parallel()
		setup := makeSetup("weaviate", weaviateConfigSDL)
	...
	t.Run("vector", func(t *testing.T) {
		...
		RunVectorScenarios(t, VectorBackendSetup{
			BackendSetup: makeSetup("weaviate_vector", vectorConfigSDL("weaviate", "{}")),
	...
	t.Run("boosting", func(t *testing.T) {
		t.Parallel()
		RunBoostingScenarios(t, makeSetup("weaviate_boosting", boostConfigSDL("weaviate", "{}")))
	})

	t.Run("date", func(t *testing.T) {
		t.Parallel()
		setup := makeSetup("weaviate_date", dateConfigSDL("weaviate", "{}"))
		setup.ExpectedResponses = map[string]string{
