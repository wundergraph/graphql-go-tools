...
func TestVariablesValidation(t *testing.T) {
	...
	t.Run("required nested enum argument provided with wrong value", func(t *testing.T) {
		...
		assert.Equal(t,
			`Variable "$input" got invalid value {"foo":"BAZ"} at "input.foo"; Value "BAZ" does not exist in "Foo" enum.`,
			err.Error())
	})
	...
	t.Run("optional enum argument provided with wrong value", func(t *testing.T) {
		...
		assert.Equal(t,
			`Variable "$bar" got invalid value "BAZ"; Value "BAZ" does not exist in "Foo" enum.`,
			err.Error())
	})
}
