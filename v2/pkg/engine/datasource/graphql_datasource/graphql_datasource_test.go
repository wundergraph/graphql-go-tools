				...
				Response: &resolve.GraphQLResponse{
					Fetches: resolve.Sequence(
						resolve.Single(&resolve.SingleFetch{
							FetchConfiguration: resolve.FetchConfiguration{
								DataSource:     &Source{},
								Input:          `{"method":"POST","url":"https://example.com/graphql","body":{"query":"{entity {__typename ... on User {__sg_merge_User_email: email} ... on Organization {__sg_merge_Organization_email: email}}}"}}`,
								PostProcessing: DefaultPostProcessingConfiguration,
							},
							DataSourceIdentifier: []byte("graphql_datasource.Source"),
						})),
									...
									Fields: []*resolve.Field{
										{
											Name: []byte("email"),
											Value: &resolve.String{
												Path: []string{"__sg_merge_User_email"},
											},
											OnTypeNames: [][]byte{[]byte("User")},
										},
										{
											Name: []byte("email"),
											Value: &resolve.String{
												Path:     []string{"__sg_merge_Organization_email"},
												Nullable: true,
											},
