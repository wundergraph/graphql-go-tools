...
//
// When the union field is resolvable by multiple candidate subgraphs, the planner
// choice between the candidates must not change the response shape, so the union
// members are shrunk to the intersection of the members across the candidates.
// Members outside the intersection are excluded from the upstream fetch;
// the members of the datasource performing the rewrite are kept in the response
// as never fetched null fields.
func TestPartialUnionComplex(t *testing.T) {
	...
	t.Run("case 1 - rootA: own member OnlyA kept as null, foreign OnlyB dropped", RunTest(
		...
		partialUnionSingleFetchPlan(
			...
			"{rootA {wrapper {actions {__typename ... on Common {label}}}}}",
			[]*resolve.Field{
				{
				...
				{
					Name:        []byte("label"),
					Value:       &resolve.String{Path: []string{"label"}, Nullable: true},
					OnTypeNames: [][]byte{[]byte("Common")},
				},
				{
					Name:        []byte("a"),
					Value:       &resolve.String{Path: []string{"a"}, Nullable: true},
					OnTypeNames: [][]byte{[]byte("OnlyA")},
				},
			},
		),
		planConfiguration,
	...
	t.Run("case 2 - rootB: own member OnlyB kept as null, foreign OnlyA dropped", RunTest(
		...
		partialUnionSingleFetchPlan(
			...
			"{rootB {wrapper {actions {__typename ... on Common {label}}}}}",
			[]*resolve.Field{
				{
					Name:  []byte("__typename"),
					Value: &resolve.String{Path: []string{"__typename"}, IsTypeName: true},
