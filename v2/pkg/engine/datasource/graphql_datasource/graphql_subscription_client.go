...
func (c *subscriptionClientV2) readLoop(ctx context.Context, msgCh <-chan *common.Message, cancel func(), updater resolve.SubscriptionUpdater) {
	...
	for {
		...
		case msg, ok := <-msgCh:
			...
			if msg.Err != nil {
				closeKind, sendErrorPayload := closeKindForMessageError(msg.Err)
				if sendErrorPayload {
...
func closeKindForMessageError(err error) (resolve.SubscriptionCloseKind, bool) {
	...
	// For non-connection errors, send GraphQL error payload and close normally.
	return resolve.SubscriptionCloseKindNormal, true
}
