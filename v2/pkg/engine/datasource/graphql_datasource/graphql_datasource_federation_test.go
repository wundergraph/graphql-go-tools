...
func TestGraphQLDataSourceFederation_PartiallySplitUnionOnShareableField(t *testing.T) {
	definition := `
		type Query {
			node(id: ID!): Node
		}
	...
	dsA := mustDataSourceConfiguration(
		...
		&plan.DataSourceMetadata{
			RootNodes: []plan.TypeField{
				{
					TypeName:   "Query",
					FieldNames: []string{"node"},
				},
				{
					TypeName:   "Node",
					FieldNames: []string{"id", "results"},
				},
			},
		...
		&plan.SynchronousResponsePlan{
			Response: &resolve.GraphQLResponse{
				Fetches: resolve.Sequence(
					resolve.Single(&resolve.SingleFetch{
						FetchConfiguration: resolve.FetchConfiguration{
							Input: `{"method":"POST","url":"http://media-a.service","body":{"query":"query($a: ID!){node(id: $a){results {__typename ... on Article {title} ... on Image {url}} __typename id}}","variables":{"a":$$0$$}}}`,
							Variables: []resolve.Variable{
					...
					resolve.SingleWithPath(&resolve.SingleFetch{
						...
						FetchConfiguration: resolve.FetchConfiguration{
							Input: `{"method":"POST","url":"http://media-b.service","body":{"query":"query($representations: [_Any!]!){_entities(representations: $representations){... on Node {__typename results {__typename ... on Video {duration}}}}}","variables":{"representations":[$$0$$]}}}`,
							Variables: []resolve.Variable{
								&resolve.ResolvableObjectVariable{
									Renderer: resolve.NewGraphQLVariableResolveRenderer(&resolve.Object{
										Nullable: true,
