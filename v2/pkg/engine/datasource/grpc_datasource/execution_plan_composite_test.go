...
func TestCompositeTypeExecutionPlan(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "Should create an execution plan for a query with a nested inner fragment followed by a complex field on the outer interface fragment",
			query: "query NestedInterfaceFragmentQuery { randomPet { ... on Animal { ... on Cat { meowVolume } habitat { region climate } } } }",
			expectedPlan: &RPCExecutionPlan{
				Calls: []RPCCall{
					{
						ServiceName: "Products",
