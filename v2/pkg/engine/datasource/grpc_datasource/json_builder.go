...
func (j *jsonBuilder) mergeValues(left *astjson.Value, right *astjson.Value) (*astjson.Value, error) {
	if len(j.indexMap) == 0 {
		...
		root, err := astjson.MergeValues(j.jsonArena, left, right)
		if err != nil {
			return nil, err
		}
		return root, nil
...
func (j *jsonBuilder) traverseList(level int, current *astjson.Value, md *ListMetadata, data protoref.Message, message *RPCMessage) (*astjson.Value, error) {
	...
	if !msg.IsValid() {
		...
		return astjson.ArrayValue(j.jsonArena), errors.New("cannot add null item to response for non nullable list")
	}
