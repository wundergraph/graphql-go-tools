...
func TestCloneSchema(t *testing.T) {
	t.Run("clone complex schema", func(t *testing.T) {
		// Create a complex schema to clone
		original := NewObjectSchema()
		original.Description = "Original schema"

		// Add properties
		original.Properties["string"] = NewStringSchema()
		...
		nested.Properties["field"] = NewStringSchema()
		original.Properties["nested"] = nested

		// Add array
		original.Properties["array"] = NewArraySchema(NewIntegerSchema())

		// Set required
		original.Required = []string{"string", "number"}
		...
		assert.Len(t, clone.Properties, len(original.Properties))
		for key, prop := range original.Properties {
			clonedProp, exists := clone.Properties[key]
			assert.True(t, exists)
			assert.Equal(t, prop.Type, clonedProp.Type)
			assert.NotSame(t, prop, clonedProp)
		}

		// Modify the clone and verify the original is unchanged
		clone.Description = "Modified clone"
		clone.Properties["string"].Description = "Modified property"
		clone.Required = append(clone.Required, "newRequired")

		assert.NotEqual(t, original.Description, clone.Description)
