...
func (b *buildScheduleTreeProcessor) ProcessFetchTree(root *resolve.FetchTreeNode) {
	...
	// DEVIATION from upstream (improve-query-order): upstream panics on any
	// scheduler/validator error, which turns a plan-time edge case into a 500 for
	// every affected operation. Fall back to the legacy wave pipeline instead —
	// the same processors that would have run with the flag off — so a failed
	// schedule degrades to today's shipping behavior, never a panic.
...
func (b *buildScheduleTreeProcessor) buildSchedule(root *resolve.FetchTreeNode) error {
...
// validateParallelLeaves enforces that two leaves scheduled in parallel do not
// depend on each other via FetchID edges.
//
// DEVIATION from upstream (improve-query-order): upstream additionally rejected
// (a) a leaf whose response path is a strict-prefix extension of a sibling's
// provided path and (b) two siblings providing the same path. Both heuristics
// FALSE-POSITIVE on real cosmo planner output, where the engine's actual
// correctness contract is complete DependsOnFetchIDs — the exact invariant
// createParallelNodes (legacy waves) and resolveParallel already rely on.
// Canonical counter-example (the hive gateways-benchmark plan): fetch 3
// (accounts, users.@.reviews.@.product.reviews.@.author, deps [1]) attaches to
// review/author objects provided ENTIRELY by fetch 1; sibling fetch 2 (products,
// users.@.reviews.@.product, deps [1]) merges product fields into a PREFIX of
// 3's path without providing 3's attach points. The planner proves the contract
// in the same plan: fetch 9 (inventory, users.@.reviews.@.product, deps [1, 2])
// DOES list fetch 2 because it actually reads fetch-2-provided data. Same-path
// siblings (heuristic b) are ordinary federation: multiple subgraphs
// contributing disjoint fields to one entity set (fetches 6 and 11 at
// topProducts). The FetchID edges are authoritative; the path heuristics are
// strictly weaker and are dropped.
