...
func TestBuildScheduleTreeScenario17ValidatorStress(t *testing.T) {
	// Fork deviation: the validator no longer infers dependencies from response-
	// path containment — FetchID edges are authoritative (complete-
	// DependsOnFetchIDs is the engine contract createParallelNodes relies on).
	// Without an edge, prefix-nested parallel siblings are VALID: an ancestor
	// fetch may have provided the nested attach points (see
	// TestBuildScheduleTreeRealGatewaysBenchmarkPlan).
	t.Run("path containment without a FetchID edge is valid", func(t *testing.T) {
		y := sfPath(0, "user")
	...
	// A real FetchID edge between parallel siblings is still rejected.
