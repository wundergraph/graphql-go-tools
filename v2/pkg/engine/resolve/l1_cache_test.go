...
// TestL1Cache_SameEntityDeduplication verifies that when the same entity is fetched
// twice within a single request, the second fetch is served from L1 cache.
// Without this, duplicate entity fetches would hit the subgraph unnecessarily.
func TestL1Cache_SameEntityDeduplication(t *testing.T) {
...
// TestL1Cache_PartialLoading verifies that with EnablePartialCacheLoad=true,
// only cache-missed entities are fetched from the subgraph.
// Without this, a single cache miss would refetch ALL entities in the batch.
func TestL1Cache_PartialLoading(t *testing.T) {
...
// TestL1Cache_PartialLoadingL1Only verifies L1-only partial loading with duplicate
// nested entities. Duplicate authors across reviews should be served from L1 cache
// instead of re-fetching from the subgraph.
func TestL1Cache_PartialLoadingL1Only(t *testing.T) {
...
// TestL1Cache_NestedEntitiesInFetchResponse verifies that nested entities within a
// fetch response are NOT extracted and cached in L1. Only the top-level fetched
// entity is cached. Without this boundary, stale nested data could be served.
func TestL1Cache_NestedEntitiesInFetchResponse(t *testing.T) {
...
// TestL1Cache_UseL1CacheFlagDisabled verifies that UseL1Cache=false on a fetch
// bypasses L1 even when L1 is globally enabled. The postprocessor sets this flag
// when a fetch cannot benefit from L1 caching.
func TestL1Cache_UseL1CacheFlagDisabled(t *testing.T) {
	t.Run("UseL1Cache=false bypasses L1 even when globally enabled", func(t *testing.T) {
		...
		// No L1 reads when UseL1Cache=false
		assert.Equal(t, 0, len(stats.L1Reads))
	})
}
