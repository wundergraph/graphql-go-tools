		...
		if fieldSelectionSetRef, ok := c.operation.FieldSelectionSet(fieldRef); ok {
			for _, ds := range c.dataSources {
				...
				for _, l1Key := range l1Keys {
					key := requestScopedGroupKey{l1Key: l1Key.L1Key, dsHash: ds.Hash()}
...
func (c *nodeSelectionVisitor) requiresConfigurationForField(ds DataSource, typeName, fieldName string) (FederationFieldConfiguration, bool) {
	requiresConfiguration, exists := ds.RequiredFieldsByRequires(typeName, fieldName)
	if exists {
		return requiresConfiguration, true
	}
	for _, io := range ds.FederationConfiguration().InterfaceObjects {
		if !slices.Contains(io.ConcreteTypeNames, typeName) {
			continue
		}
		requiresConfiguration, exists = ds.RequiredFieldsByRequires(io.InterfaceTypeName, fieldName)
		if exists {
